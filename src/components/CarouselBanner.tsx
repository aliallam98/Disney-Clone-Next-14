"use client";

import { Swiper, SwiperSlide } from "swiper/react";
import 'swiper/css';

// import required modules
import { Autoplay } from "swiper/modules";
import Image from "next/image";
import { Movie } from "../../typings";
import { getImagePath } from "@/lib/getImagePath";

type props = {
  movies: Movie[];
};

export default function CarouselBanner({ movies }: props) {
  return (
    <>
      <div className="overflow-hidden lg:-mt-40 relative ">
        <Swiper
          spaceBetween={30}
          centeredSlides={true}
          autoplay={{
            delay: 8000,
            disableOnInteraction: false,
          }}
          loop
          speed={3000}

          modules={[Autoplay]}
          className="mySwiper"
        >
          {movies.map((movie) => (
            <SwiperSlide key={movie.id}>
              <Image
                src={getImagePath(
                  movie.backdrop_path || movie.poster_path,
                  true
                )}
                alt={movie.title}
                width={1920}
                height={1080}
              />
              <div className="hidden lg:inline absolute mt-0 top-0 pt-40 xl:pt-52 left-0 lg:mt-40 bg-transparent z-20 h-full w-full bg-gradient-to-r from-gray-900/90 via-transparent to-transparent p-10 space-y-5 text-white ">
                <h2 className="text-5xl font-bold max-w-xl z-50">
                  {movie.title}
                </h2>
                <p className="max-w-xl line-clamp-3">{movie.overview}</p>
              </div>
            </SwiperSlide>
          ))}
        <div className="absolute inset-0 bg-gradient-to-b from-gray-200/0 via-gray-900/25 to-gray-300 dark:to-[#1A1C29]" />
        </Swiper>

        <div className="absolute inset-0 z-20 bg-gradient-to-b from-gray-200/0 via-gray-900/25 to-gray-300 dark:to-[#1A1C29]" />
      </div>
    </>
  );
}
